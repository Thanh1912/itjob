...
export class CompanyComponent implements OnInit {
  public uploader_logo: FileUploader = new FileUploader({
    url: "http://localhost:3000/api/uploadlogo",
    itemAlias: "file_logo"
  });
  public uploader_co: FileUploader = new FileUploader({
    url: "http://localhost:3000/api/uploadcogty",
    itemAlias: "file_co"
  });
  ...
  constructor(
    ...
    private company: CompanysizeService,
    private title: Title
  ) {}
  ...
  ngOnInit() {
    ...
    this.title.setTitle("Manager infomation Company");
    this.id_user = localStorage.getItem("userId_ntd");
    this.load_info_user(this.id_user);
    this.getall_();
    this.getall_company();
    //override the onAfterAddingfile property of the uploader so it doesn't authenticate with //credentials.
    this.uploader_logo.onAfterAddingFile = file => {
      file.withCredentials = false;
    };
    //overide the onCompleteItem property of the uploader so we are
    //able to deal with the server response.
    this.uploader_logo.onCompleteItem = (
      item: any,
      response: any,
      status: any,
      headers: any
    ...
    //override the onAfterAddingfile property of the uploader so it doesn't authenticate with //credentials.
    this.uploader_co.onAfterAddingFile = file => {
      file.withCredentials = false;
    };
    //overide the onCompleteItem property of the uploader so we are
    //able to deal with the server response.
    this.uploader_co.onCompleteItem = (
      item: any,
      response: any,
      status: any,
      headers: any
  ...
  load_info_user(id_user) {
    this.QuanliNtdService.getinfo(id_user).subscribe(
      data => {
        ...
        this.congtyForm.controls["profileimage"].setValue(
          json.info_recruiter.profileimage
        );
        this.congtyForm.controls["website"].setValue(
          json.info_recruiter.website
        );
        this.congtyForm.controls["namecompany"].setValue(
          json.info_recruiter.namecompany
        );
        this.congtyForm.controls["facebook"].setValue(
          json.info_recruiter.facebook
        );
        this.congtyForm.controls["phone"].setValue(json.info_recruiter.phone);
        this.congtyForm.controls["address"].setValue(
          json.info_recruiter.address
        );
        this.congtyForm.controls["introduction"].setValue(
          json.info_recruiter.introduction
        );
        this.congtyForm.controls["countryid"].setValue(
          json.info_recruiter.countryid
        );
        this.congtyForm.controls["companysizeid"].setValue(
          json.info_recruiter.companysizeid
        );
        this.congtyForm.controls["countryid"].setValue(
          json.info_recruiter.countryid
        );
