  ...
  var upload = multer({
    ...
    fileFilter: function(req, file, callback) {
      ...
      if (
        ...
        return callback(res.status(403).end("Only images are allowed"), null);
      }
      callback(null, true);
    }
  }).single("anhdaidien");
...
router.post("/api/uploadcv", function(req, res) {
  var upload = multer({
    ...
    fileFilter: function(req, file, callback) {
      var ext = path.extname(file.originalname);
      if (ext !== ".pdf") {
        return callback(res.status(403).end("Only PDF are allowed"), null);
      }
      callback(null, true);
    }
  }).single("file_cv");
  upload(req, res, function(err) {
    if (typeof req.file === "undefined") {
      return res.end("Only PDF are allowed");
    }
    return res.end(req.file.filename);
  });
});
